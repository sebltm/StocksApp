...
//        SEARCH ENDPOINT KEEPS CHANGING RESULTS (NOT FIXED RESULTS), SOMETIMES AAPL DOESN'T SHOW UP IN RESULTS
    ...
    public void FETCH_data_TEST() {
        ...
        BiConsumer<HashMap<String, Object>, Runnable> func = (stock_info, done) -> {
...
//                        System.out.println(String.format("API ERROR, DATA NOT FOUND FOR SYMBOL: %s", s.getSymbol()));
                        ...
                        priceHistory.forEach((pp) -> {
                            arePricePointValuesSizeSame(pp);
        ...
        executeTest(done -> {
            func.accept(new HashMap<String, Object>() {{
                put("currency", "USD");
    ...
    private void SMA_EMA_DEFAULT_TEST(List<PricePoint> price_points, Stock stock, AnalysisType a_type,
                                      Function<Integer, List<AnalysisPoint>> calc, Function2<Integer, Integer, Boolean> validate) {
        assertNotNull("Stock is not null", stock);
        if (PricePointsNotFound(price_points)) {
            return;
        }
        price_points.forEach(this::arePricePointValuesSizeSame);
    ...
    private void MACD_MACDAVG_DEFAULT_TEST(List<PricePoint> price_points, Stock stock,
