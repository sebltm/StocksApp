...
public class SearchActivity extends AppCompatActivity {
            ...
            if (fromDate.getCal().compareTo(toDate.getCal()) <= 0) {
                ...
                selectedStock.fetchData(
                        ...
                        (price_points, stock) -> {
                            ...
                            ANALYSIS_POINTS.clear();
                            ANALYSIS_POINTS.putAll(stock.calculateSelectedIndicators(
                                    analysisTypes,
                                    fromDateTime,
                                    toDateTime,
                                    nDays,
                                    errors -> errors.forEach(error -> showToastThread(error,Toast.LENGTH_SHORT))
                            ));
                            System.out.println(ANALYSIS_POINTS);
    ...
    private void showToastThread(String msg, int duration) {
        runOnUI(() -> Toast.makeText(SearchActivity.this, msg, duration).show());
    }
}